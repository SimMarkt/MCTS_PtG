...
class MCTSNode:
    ...
    def best_child(self, c_param=1.41):
        choices_weights = [
            (child.total_reward / child.visits if child.visits > 0 else 0) + 
...
class MCTS:
    def __init__(self, iterations=1000, exploration_weight=1.41, num_workers=None):
        self.iterations = iterations
        self.exploration_weight = exploration_weight
        self.num_workers = num_workers or cpu_count()  # Use all available CPU cores by default

    def search(self, root_env, rootnodeid):
        print(f"[DEBUG] Starting MCTS search with {self.iterations} iterations and {self.num_workers} workers.")
        root_env_copy = copy.deepcopy(root_env)
        root_node = MCTSNode(root_env_copy)

        with Pool(self.num_workers, initializer=self._worker_init) as pool:
            results = pool.map(self._rollout, [(root_node, root_env_copy, i) for i in range(self.iterations)])

        for reward, node in results:
            self._backpropagate(node, reward)

        print("[DEBUG] MCTS search completed.")
        return root_node.most_visited_child().action

    @staticmethod
    def _worker_init():
        """Initializer for worker processes."""
        print(f"[DEBUG] Worker {current_process().name} initialized.")

    def _rollout(self, args):
        root_node, root_env_copy, rollout_id = args
        print(f"[DEBUG] Worker {current_process().name} performing rollout {rollout_id}.")
        node = self._select(root_node)
        if not node.is_terminal():
            node = self._expand(node)
        reward = self._simulate(node.env, node.done, node.action, node.remaining_steps)
        print(f"[DEBUG] Worker {current_process().name} completed rollout {rollout_id} with reward {reward}.")
        return reward, node
