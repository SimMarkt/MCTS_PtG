...
class MCTS:
    ...
    def search(self, root_node):
        """
        Perform MCTS starting from the given root node.
...
def print_tree(node, depth=0, c_param=1.41):
    """
    Recursively prints the tree structure with actions as children.
    :param node: The current node in the tree.
    :param depth: The depth of the current node (used for indentation).
    """
    indent = "  " * depth
    print(f"{indent}Action: {node.action}, Visits: {node.visits}, Total Reward: {node.total_reward}")
...
def main():
    ...
    stats_dict_test = {}
    root_node = MCTSNode(env_test_post)  # Initialize the root node for MCTS

    for i in tqdm(range(timesteps), desc='---Apply MCTS planning on the test environment:'):
        # Perform MCTS search to get the best child node
        ...
        # Update the root node to the selected child node
        root_node = best_child
        root_node.parent = None  # Detach the new root node from its parent to avoid memory leaks

        # # Print the tree structure
        # print("\nTree structure CHILDNODE:")
        # print_tree(root_node)
