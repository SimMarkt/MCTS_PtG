...
class MCTS:
    ...
    def search(self, root_env, rootnodeid):
        ...
        with mp.Pool(processes=mp.cpu_count()) as pool:
            for g in range(self.iterations):
                ...
                tasks = [
                    (worker_id, copy.deepcopy(node.env), node.done, node.action, node.remaining_steps)
                    for worker_id in range(mp.cpu_count())
                ]

                # Parallelize the simulation step
                simulation_results = pool.starmap(self._simulate, tasks)

                reward = max(simulation_results)  # Use the best reward from parallel simulations
    ...
    def _simulate(self, worker_id, env, done, action, remaining_steps):
        """
        Simulates a rollout from the given environment state.

        :param worker_id: ID of the worker performing the simulation
        :param env: Environment to simulate
        :param done: Whether the environment is in a terminal state
        :param action: Action to take in the simulation
        :param remaining_steps: Number of steps remaining in the simulation
        :return: Total reward from the simulation
        """
        print(f"Worker {worker_id}: Starting simulation with action {action} and remaining steps {remaining_steps}")
        ...
        print(f"Worker {worker_id}: Finished simulation with total reward {total_reward}")
        return total_reward
